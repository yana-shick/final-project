...
export default function OneRoom(props) {
	...
	const showPlus = () => {
		...
		return (
			<img
				style={{ display: isDeviceListShown[1] }}
				onClick={() => setIsDeviceListShown([true, "none"])}
				src={Plus}
				alt="PlusIcon"
			/>
		);
		...
		if (isDeviceListShown[0]) {
			...
			return (
				<div className="container">
					...
					{/* Add button */}
					<div
						className="one-room-add-button"
						onClick={() => {
							let choosenDevice =
								document.getElementById("deviceListSelect").value;
							data.addDeviceFunc(choosenDevice, props.room.id);
							if (props.room.deviceArr.length == 6) {
		...
		<div className="one-room-page">
			<div className="one-room-top">
				...
				<div className="one-room-header">
					...
					{/* delete Button */}
					<div
						className="one-room-erase-button"
						onClick={() => {
							data.eraseRoomFunc(props.room.id);
							nav("/final-project");
						}}
					>
				...
				<DeviseInRoom
					devisesInRoom={props.room.deviceArr}
					id={props.room.id}
					setIsDeviceListShown={setIsDeviceListShown}
				/>
			</div>
